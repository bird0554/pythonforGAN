...
            self.pre_opt = optimizer(self.pre_loss, None, self.learning_rate)#使用衰减后的学习率，最小化loss，获得预先训练的网络模型，用其来初始化判别网络。
...
        self.loss_d = tf.reduce_mean(-tf.log(self.D1) - tf.log(1 - self.D2))#判别网络损失函数#(-tf.log(self.D1) - tf.log(1 - self.D2))
...
                d = (np.random.random(self.batch_size) - 0.5) * 10.0#（判别网络真实数据输入值）随机生成初始数据，numpy.random.random(size = None): Return random floats in the half-open interval [0.0, 1.0),size一个参数时对应（列数），两个参数时对应（行，列）
                labels = norm.pdf(d, loc=self.data.mu, scale=self.data.sigma)#（判别网络真实数据输出值）根据d进行高斯值的生成，norm.pdf(x, loc, scale) is identically equivalent to norm.pdf(y) / scale with y = (x - loc) / scale.
                pretrain_loss, _ = session.run([self.pre_loss, self.pre_opt], {#pretrain_loss, _ =用来接收返回值，如果返回值无用可以用 _ 简单表示
...
    parser.add_argument('--num-steps', type=int, default=1200,#1200#生成网络训练次数
...
————————————————
版权声明：本文为CSDN博主「羽神之念」的原创文章，遵循 CC 4.0 BY-SA 版权协议，转载请附上原文出处链接及本声明。
原文链接：https://blog.csdn.net/Wei_Xiaoyu/article/details/79711236